...
\citeauthor{AOSPsecurity}~\cite{AOSPsecurity} describes channels designed for inter-app communication, such as sandbox sharing and binder, on pages \href{https://source.android.com/security/overview/app-security}{``Application security''} and \href{https://source.android.com/security/app-sandbox}{``Application Sandbox''}.
...
\subsection{Detecting collusion that uses overt channels}

\begin{itemize}
	\item \subsubsection{Based on permissions/interfaces}
	\item \subsubsection{Based on control flow analysis}
	\item \subsubsection{Heuristics}
	\item \subsubsection{Manual analysis}
\end{itemize}

\subsection{Detecting collusion that uses covert channels}

No such work exists.

People have looked into closing some covert channels, but it is not reasonable to assume that all covert channels can be found. Additionally, even today there exist known open covert channels.
