...
\begin{enumerate}
	...
	\begin{singleitems}
		\item Intents (each action separately):\\
			Send: API call to send intent exists in code\\
			Receive: intent filter defined in manifest or registered in code
		\item External storage:\\
			Send: WRITE\_EXTERNAL\_STORAGE\\
			Receive: READ\_EXTERNAL\_STORAGE
		\item Shared preferences (each file separately):\\
			Send: API call to write exists in code\\
			Receive: API call to read exists in code
	\end{singleitems}
\end{enumerate}
